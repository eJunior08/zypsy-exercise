...
We want you to develop a posts' list feature with category filtering. Posts are accessible by category. Posts can have many categories.

- When clicking on a category, the category's posts are displayed. Selected category should be maintained if the user reloads the page in the browser
- When clicking on a star icon, the category is marked as favorite
...
We provide you with a ready design. It is required to implement the feature according to the design.
...
The project requires **node >=22**, so check if you have the correct version. 
